...
The app bundles three tabs that exercise different parts of the
schema:

-   **Active Players** — inner-joins `player` with `player_photos` on
    `idPlayer`, filters to `is_active = 1`, and renders circular
    headshot cards from the `urlPlayerHeadshot` column. A slider
    controls how many players to display (1–519, the full active
    roster).
-   **Top Salaries** — aggregates `player_salary` by `namePlayer` using
    `MAX(value)` to get each player's highest contract, then ranks
    descending. A slider controls Top-N (5–30).
-   **Active vs Retired** — counts `is_active` in the `player` table
    (519 active vs 3,982 retired) and shows both a bar chart and a
    table.

The same color palette as Assignment 7 (`#18A3A380` fill,
`#FF4D8DCC` accent) is reused so the look matches the rest of the
site.
...
<iframe src="https://chentzuyuan.shinyapps.io/epps6354-hw8-nba/" width="100%" height="800" style="border:1px solid #ccc; border-radius:8px;" loading="lazy"></iframe>
...
#### Database Tables (Simplified — 7 Tables)

The original design used 21 normalized tables. To reduce complexity while preserving all core
functionality, the schema was consolidated into 7 tables by: (1) removing the `User` table
(single-user system), (2) absorbing lookup tables (Category, Color, Fabric, StyleTag) into
JSON columns on `ClothingItem`, (3) merging detail tables into their parent tables, and
(4) unifying wear tracking and laundry into a single event log.
...
erDiagram
    ...
    Occasion ||--o{ CalendarEvent : "has"
    Occasion ||--o{ Outfit : "generates"
    Weather ||--o{ Outfit : "conditions"
    Outfit ||--o{ WearLog : "records"
    ClothingItem ||--o{ WearLog : "tracks"
    ScoringRule }o--o{ Outfit : "evaluates"
```
...
| `name` | VARCHAR(50) | Item name (e.g. "Navy Oxford Shirt") |
| `category` | VARCHAR(20) | Category: top, bottom, shoes, outerwear, accessory |
| `fabric` | VARCHAR(20) | Fabric type (e.g. cotton, denim, linen, polyester) |
| `warmth_weight` | INT | Warmth rating (0–100) |
| `breathability` | INT | Breathability rating (0–100) |
| `colors` | JSON | Color roles: `[{"name":"navy","role":"primary"},{"name":"white","role":"accent"}]` |
| `styles` | JSON | Style tags: `["clean fit","formal"]` |
| `formality_score` | INT | Formality level (0–100) |
| `clean_score` | INT | Cleanliness (100 = fresh, 0 = needs wash); item is recommendable only when > 0 |
| `status` | VARCHAR(10) | `active` or `archived`; only active items enter recommendation |

*Absorbs the former: User, Category, Color, Fabric, StyleTag, ItemStyle, ItemColor.*

##### 2. Occasion

| Column | Type | Description |
|--------|------|-------------|
| `occasion_id` | INT PK | Unique identifier |
| `name` | VARCHAR(30) | Occasion name (e.g. "job interview", "casual outing", "gym") |
| `formality_min` | INT | Minimum acceptable formality (0–100) |
| `formality_max` | INT | Maximum acceptable formality (0–100) |
| `style_fit` | JSON | Style-to-fit-score mapping: `{"formal":95,"clean fit":80,"street":20}` |

*Absorbs the former: Occasion, StyleOccasionFit. The `style_fit` JSON replaces the separate many-to-many table.*
...
| `occasion_id` | INT FK → Occasion | Links to the occasion type |
| `event_date` | DATE | Date of the event |
| `start_time` | TIME | Start time; used to determine morning vs. evening outfit needs |

*Drives automatic occasion detection: the system reads today's events to determine the occasion context.*
...
| `condition_name` | VARCHAR(20) | Named condition (e.g. "cold_rainy", "hot_humid", "mild_clear") |
| `temp_c` | DECIMAL(4,1) | Temperature in Celsius |
| `feels_like_c` | DECIMAL(4,1) | Feels-like temperature |
| `humidity` | INT | Humidity percentage (0–100) |
| `wind_speed` | DECIMAL(4,1) | Wind speed in km/h |
| `precip_mm` | DECIMAL(5,1) | Precipitation in mm |

*Merges the former: WeatherSnapshot, WeatherCondition. The `condition_name` column replaces the separate condition lookup table.*

##### 5. Outfit

| Column | Type | Description |
|--------|------|-------------|
| `outfit_id` | INT PK | Unique identifier |
| `occasion_id` | INT FK → Occasion | The occasion this outfit was generated for |
| `weather_id` | INT FK → Weather | The weather snapshot at generation time |
| `items` | JSON | Ordered item list: `[{"item_id":3,"slot":"top","layer":1},{"item_id":7,"slot":"bottom","layer":1}]` |
| `color_score` | INT | Color harmony sub-score (60/30/10 rule) |
| `fabric_score` | INT | Fabric compatibility sub-score |
| `style_score` | INT | Style consistency sub-score |
| `total_score` | INT | Weighted total of the three sub-scores |
| `explanation` | TEXT | Human-readable scoring breakdown |

*Merges the former: Outfit, OutfitItem. The `items` JSON replaces the separate detail table, supporting multi-layer dressing via the `layer` field.*
...
| `rule_type` | VARCHAR(20) | `outfit_score` (scoring bonus/penalty) or `wear_degrade` (clean score deduction) |
| `description` | VARCHAR(100) | Human-readable rule description (e.g. "Same-fabric bonus +5") |
| `condition_json` | JSON | Condition to match: `{"category":"top","occasion":"gym"}` |
| `score_delta` | INT | Points to add/subtract when condition is met |
| `valid_from` | DATE | Rule effective start (nullable; supports trend-based rules) |
| `valid_until` | DATE | Rule expiry date (nullable) |

*Merges the former: OutfitScoringRule, WearDegradeRule. The `rule_type` column distinguishes between outfit scoring rules and wear degradation rules.*
...
| `item_id` | INT FK → ClothingItem | The clothing item involved |
| `outfit_id` | INT FK → Outfit | Associated outfit (nullable; null for laundry events) |
| `event_type` | VARCHAR(10) | `wear` or `laundry` |
| `event_date` | TIMESTAMP | When the event occurred |
| `delta_clean` | INT | Clean score change (negative for wear, positive for laundry reset to 100) |
| `clean_score_after` | INT | Resulting clean score after this event |

*Merges the former: WearEvent, WearEventItem, LaundryBatch, LaundryBatchItem. Each row records one item's wear or laundry event. Laundry resets `clean_score` to 100.*
