...
App 設計成三個 tab，分別操作不同的表：

-   **Active Players** — 用 `idPlayer` 將 `player` 與 `player_photos`
    做 inner join，過濾 `is_active = 1`，並從 `urlPlayerHeadshot`
    欄位渲染圓形頭像卡片。slider 控制顯示人數（1–519，現役球員全數）。
-   **Top Salaries** — 用 `MAX(value)` 對 `player_salary` 按
    `namePlayer` 聚合，取出每位球員的最高合約，再降序排列。slider
    控制 Top-N（5–30）。
-   **Active vs Retired** — 統計 `player` 表的 `is_active`
    （現役 519 人 vs 退役 3,982 人），同時顯示長條圖與表格。

採用與作業 7 相同的配色（`#18A3A380` 填色、`#FF4D8DCC`
邊框），與整個網站風格一致。
