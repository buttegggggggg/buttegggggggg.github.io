...
| 資料表 | 主要欄位 | 說明 |
|-------|-------------|-------------|
| `User` | `user_id` PK, `name` | 儲存使用者身分 |
| `Category` | `category_id` PK, `name` | 衣物類別：上衣、下裝、鞋子、外套、配件 |
| `Color` | `color_id` PK, `name` | 顏色名稱（黑、白、海軍藍、灰、米色...） |
| `Fabric` | `fabric_id` PK, `name`, `warmth_weight`, `breathability` | 布料類型，含保暖度 (0-100) 和透氣度 (0-100) 評分 |
| `StyleTag` | `style_id` PK, `name` | 風格標籤：街頭風、正式、簡約風、簡單、blokecore... |
| `Occasion` | `occasion_id` PK, `name`, `target_formality_min`, `target_formality_max` | 場合及其要求的正式度範圍 |

##### 風格與行事曆

| 資料表 | 主要欄位 | 說明 |
|-------|-------------|-------------|
| `StyleOccasionFit` | `style_id` FK, `occasion_id` FK, `fit_score` | 風格與場合之間的適配分數 (0-100)（PK: style_id + occasion_id） |
| `CalendarEvent` | `event_id` PK, `user_id` FK, `occasion_id` FK, `event_date`, `start_time` | 對應到場合的行事曆項目；驅動自動場合偵測 |

##### 衣物項目與標籤

| 資料表 | 主要欄位 | 說明 |
|-------|-------------|-------------|
| `ClothingItem` | `item_id` PK, `user_id` FK, `category_id` FK, `fabric_id` FK, `formality_score`, `warmth_score`, `clean_score`, `status` | 主要衣物資料表；僅在 `clean_score > 0` 且 `status = 'active'` 時可推薦 |
| `ItemStyle` | `item_id` FK, `style_id` FK | 多對多：衣物項目到風格標籤 |
| `ItemColor` | `item_id` FK, `color_id` FK, `role` (primary/secondary/accent) | 每件衣物的顏色角色分配；每件衣物最多 3 個顏色 |

##### 天氣

| 資料表 | 主要欄位 | 說明 |
|-------|-------------|-------------|
| `WeatherSnapshot` | `weather_id` PK, `location`, `temp_c`, `feels_like_c`, `humidity`, `wind_speed`, `precip_mm` | 即時天氣資料快照 |
| `WeatherCondition` | `condition_id` PK, `name`, temp/precip/wind/humidity ranges | 命名的天氣條件（如 cold_rainy、hot_humid），用於規則匹配 |

##### 穿搭

| 資料表 | 主要欄位 | 說明 |
|-------|-------------|-------------|
| `Outfit` | `outfit_id` PK, `user_id` FK, `occasion_id` FK, `weather_id` FK, `total_score`, `explanation` | 產生的穿搭，含總分和說明 |
| `OutfitItem` | PK(outfit_id, slot, layer_order), `item_id` FK | 穿搭細節；支援同一位置的多層穿著 |

##### 評分規則

| 資料表 | 主要欄位 | 說明 |
|-------|-------------|-------------|
| `OutfitScoringRule` | `score_rule_id` PK, `rule_type`, `score_delta`, `condition_json`, `valid_from`, `valid_until` | 評分規則，含可選的到期日期以支援趨勢性規則 |
| `WearDegradeRule` | `degrade_rule_id` PK, `category_id` FK, `occasion_id` FK (nullable), `condition_id` FK (nullable), `delta_clean_score` | 清潔度扣分規則；每次穿著事件可疊加多條匹配規則 |

##### 穿著追蹤與洗衣

| 資料表 | 主要欄位 | 說明 |
|-------|-------------|-------------|
| `WearEvent` | `event_id` PK, `user_id` FK, `outfit_id` FK, `occasion_id` FK, `weather_id` FK, `worn_at` | 記錄每次穿著穿搭的事件 |
| `WearEventItem` | PK(event_id, item_id), `delta_applied`, `clean_score_after` | 每件衣物的扣分細節及穿著後的清潔度分數 |
| `LaundryBatch` | `batch_id` PK, `user_id` FK, `laundry_type` (dark/light) | 按顏色類型分組的洗衣批次 |
| `LaundryBatchItem` | PK(batch_id, item_id), `reset_to_score` = 100 | 洗衣後將清潔度分數重設為 100 |
